...
func GetMovie(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			fmt.Printf("Error on find movie:  %v\n", err)

			c.JSON(http.StatusNotFound, gin.H{"error": "Error fetching movie from database"})
			return
		}
...
func GetRankings(client *mongo.Client, c *gin.Context) ([]models.Ranking, error) {
	...
	if err := cursor.All(ctx, &rankgins); err != nil {
		return nil, err
	}
...
func GetRecommendedMovies(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if recommendedMovieLimitStr != "" {
			recommendedMoviesLimit, _ = strconv.ParseInt(recommendedMovieLimitStr, 10, 64)
		}
